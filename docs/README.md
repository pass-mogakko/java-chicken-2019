...
1. - [x] 모든 테이블 목록 정보를 조회하는 기능
   - 각 테이블의 주문 내역의 존재 여부를 조회 가능
2. - [x] 모든 메뉴 정보를 조회하는 기능
   - `예외` null 또는 비어있는 메뉴 정보
3. - [x] 테이블 번호, 주문 정보를 전달받아 주문 내역 목록을 갱신하는 기능
   - 대기 중인 주문 내역이 존재하면 갱신, 없으면 새로 저장한다.
   - 주문 내역에 이미 동일한 메뉴에 대한 주문이 있으면 수량을 갱신한다.
   - `예외` 존재하지 않는 테이블 번호
   - `예외` 존재하지 않는 메뉴 번호
   - `예외` 한 메뉴에 대한 수량이 99를 초과
4. - [x] 테이블 번호를 전달받아 해당 테이블의 주문 내역 목록을 조회하는 기능- 
   - `예외` 존재하지 않는 테이블 번호
5. - [x] 결제 유형, 테이블 번호를 전달받아 결제하는 기능
   - 결제 완료 시 해당 테이블의 주문 상태를 완료로 바꾸고, 주문 내역을 초기화시킨다.
   - 결제 유형, 해당 테이블 번호의 주문 정보를 이용해 최종 결제 금액을 계산한다.
   - `예외` 존재하지 않는 테이블 번호
...
6. - [x] 메뉴 선택을 입력받는 기능
   - `예외` 숫자가 아닌 경우
7. - [x] 선택한 메뉴의 수량을 입력받는 기능
   - `예외` 자연수가 아닌 경우
8. - [x] 주문 내역을 출력하는 기능
   - `예외` 출력할 값이 null인 경우
9. - [x] 결제 유형 선택을 입력받는 기능
   - `예외` 지정된 선택 입력값이 아닌 경우
10. - [x] 최종 결제 금액을 출력하는 기능
11. - [x] 잘못된 입력값에 대하여 안내메시지 출력 후 재입력받는 기능
...
- Domain-4: 주문 조회
  - [x] 성공 테스트
    - Order와 OrderDTO 비교
    - 아무 주문도 하지 않은 경우 빈 리스트 반환
  - [x] 예외 테스트
    - `예외` 존재하지 않는 테이블 번호
- Domain-5: 결제
  - [x] 성공 테스트
    - 주문 내역 초기화
    - 총 금액 계산 (수량에 따라 할인 적용)
    - 최종 결제 금액 계산 (결제 금액에 따라 할인 적용)
  - [x] 예외 테스트
    - `예외`  존재하지 않는 테이블 번호
    - `예외`  존재하지 않는 주문 내역
## 리팩토링 목록
